...
DECLARE_MULTICAST_DELEGATE(FOnRestrictStartDelegate)//무기 스왑관련 델리게이트 멀티캐스트로 선언 명령어:Broadcast(전체파일)
DECLARE_MULTICAST_DELEGATE(FOnRestrictEndDelegate)//맨손 공격관련 델리게이트 멀티캐스트로 선언 명령어:Broadcast(전체파일)
...
public:
	...
	UFUNCTION()
		void AnimNotify_RestrictStart();//어택 첵
	UFUNCTION()
		void AnimNotify_RestrictEnd();//어택 첵
};
