...
AShooter2Character::AShooter2Character()
{
	...
	FollowCamera->SetRelativeRotation(FRotator(0.0f, 0.0f, 0.0f));//카메라 회전
...
void AShooter2Character::BeginPlay()
{
	...
	{
		//플레이어 컨트롤러 설정
		ShooterPlayerController = Cast < AShooterCharacterPlayerController >(GetController());
	}
...
void AShooter2Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// "turn"은 마우스와 같이 절대 델타를 제공하는 장치를 처리합니다
...
void AShooter2Character::Tick(float DeltaTime)
{
	...
	//총을 안쓰거나 달리는 중 이거나 공격중이 아닐 때
	if (!ShooterAnim->IsGunUsed() && !bIsAttacking || ShooterAnim->IsSprinting() && !ShooterAnim->IsAiming())
	{
		GetCharacterMovement()->bOrientRotationToMovement = true;//이동방향으로의 캐릭터회전을 킵니다
	}
	else
	{
		GetCharacterMovement()->bOrientRotationToMovement = false;//이동방향으로의 캐릭터회전을 끕니다
...
void AShooter2Character::MyJump()
{
	...
	//앉았을 때 점프시 앉기 해제
	if (ShooterAnim->MyIsCrouching() == true)
	{
...
void AShooter2Character::Aim()//에임 온
{
	...
	//에임중 속도 제어
	if (ShooterAnim->IsAiming() == false && ShooterAnim->IsGunUsed() == true)
	{
		ShooterAnim->SetAiming(true);
		GetCharacterMovement()->MaxWalkSpeed = StandSpeed;
		UE_LOG(Shooter2, Error, TEXT("Aim On"));
	}
}
void AShooter2Character::Aimoff()//에임 오프
{
	...
	//에임중 속도 제어
	if (ShooterAnim->IsAiming() == true )
	{
		ShooterAnim->SetAiming(false);

		//달리기를 누르는 중에 에임이 꺼지면 달리기 속도로
		if (ShooterAnim->IsSprinting() == true)
		{
			GetCharacterMovement()->MaxWalkSpeed = StandRunSpeed;
		}
		UE_LOG(Shooter2, Error, TEXT("Aim Off"));
	}
}
...
void AShooter2Character::Reload()//재장전 몽타주 실행
{
	if (ShooterAnim->IsGunUsed() == true && !ShooterAnim->UAnimInstance::IsAnyMontagePlaying() && CharacterStat->GetMagazineBullet() <30)
	{
		...
		if (CharacterStat->GetMagazineBullet() > 0)
		{
			...
			//소지총알이 장전에 필요한 총알수보다 많은 경우
			if (CharacterStat->GetMaxBullet() >= NeedToAmmo && CharacterStat->GetMaxBullet() > 0)
			{
				CharacterStat->SetMagazineCount(NeedToAmmo);//필요한 총알 수만큼 탄창에 추가
				CharacterStat->SetMaxBullet(-NeedToAmmo);//필요한 총알 수만큼 소지총알 추감
			}
			//소지총알이 장전에 필요한 총알수보다 적은 경우
			else if (CharacterStat->GetMaxBullet() < NeedToAmmo && CharacterStat->GetMaxBullet()>0)
			{
		...
		//탄장에 총알이 비어 있을 때
		else if (CharacterStat->GetMagazineBullet() <= 0)
		{
			int32 NeedToAmmo = 30;
			//소지총알이 장전에 필요한 총알수보다 많은 경우
			if (CharacterStat->GetMaxBullet() >= NeedToAmmo && CharacterStat->GetMaxBullet() > 0)
...
void AShooter2Character::Fire()//총 발사
{
	...
	if (CharacterStat->GetMagazineBullet() > 0)
	{
		...
		FVector PlayerViewPointRotationVector = PlayerViewPointRotation.Vector();//뷰포트 회전값을 벡터화
		PlayerViewPointRotationVector *= 10000.0f;//길이설정

		FVector End = PlayerViewPointLocation + PlayerViewPointRotationVector;//라인트레이스 끝점 설정
		...
		//라인 트레이스로 히트확인검사
		bool Result = GetWorld()->LineTraceSingleByObjectType(HitRes, PlayerViewPointLocation, End, ObjQuery, ColQuery);

		if (Result)//(HitRes.bBlockingHit)//결과 인스턴스 유효할 시 
		{
			...
			//유효한 액터는 데미지를 전달하고 내 위치를 상대 데미지 포인트에 지정,
			//약포인터 방식으로 메모리를 참조하기 때문에 메모리 낭비를 방지
		...
		if (AgroResult)//AI판단 및 어그로 활성화
		{
			...
			{
				//AI캐릭터일 경우 TrackToTargetOutRange호출
				if (OverLapResult.GetActor()->GetClass()->IsChildOf(AShooterAICharacter::StaticClass()))
				{
					auto ResultAI = Cast<AShooterAICharacter>(OverLapResult.GetActor());
					auto AIController = Cast<AShooterAIController>(ResultAI->GetController());
		...
		if (!CurrentGun->GetMode() && bIsAttacking)//연사모드일 시 타이머를 이용한 재귀호출을 하여 연사를 구현
		{
	...
	{
		Reload();//탄창에 총알이 없고 예비총알을 소지중 일시 리로드
	}
	else if (CharacterStat->GetMagazineBullet() <= 0 && CharacterStat->GetMaxBullet() <= 0)
	{
		CurrentGun->ModeChangeSound->Play();//탄창에 총알이 없고 예비총알도 없으면 빈 총소리
	}
}
...
void AShooter2Character::MeleeAttack()//근접공격
{
	...
	//공격 결과를 담음 (채널 방식으로구현 : 해당 트레이스 채널로 검사를 실행후 결과를 반환)
...
void AShooter2Character::CharacterDead()
{
	...
	GetMesh()->SetCollisionProfileName(TEXT("Ragdoll"));//메스시 콜리전을 래그돌로설정
...
//해당액터에 데미지를 가한 액터 위치 설정 (넉백 방향 설정용)
void AShooter2Character::SetDamagePoint(FVector DamageSource)
{
...
void AShooter2Character::PostInitializeComponents()
{
	...
	ShooterAnim->OnWeaponSwap.AddUObject(this, &AShooter2Character::SwapWeapon);//노피파이 확인시 무기스왑 작동
	...
	GetCapsuleComponent()->OnComponentBeginOverlap.AddDynamic(this, &AShooter2Character::OnSupplyBoxOverLap);//오버랩을 처리하는 델리게이트 함수(서플라이)
	GetCapsuleComponent()->OnComponentEndOverlap.AddDynamic(this, &AShooter2Character::OnSupplyBoxEndOverLap);//오버랩을 처리하는 델리게이트 함수(서플라이)
	...
	const FHitResult& SweepResult)//오버랩 이벤트를 처리하는 함수(서플라이박스)
{
	...
	{
		UE_LOG(Shooter2, Error, TEXT("Supply Overlap Begin"));
	...
	int32 OtherBodyIndex)//오버랩 이벤트를 처리하는 함수(서플라이박스)
{
	...
	{
		UE_LOG(Shooter2, Error, TEXT("Supply Overlap End"));
