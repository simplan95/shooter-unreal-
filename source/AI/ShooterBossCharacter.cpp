...
AShooterBossCharacter::AShooterBossCharacter()
{
	...
	WidgetDeadTimer = 2.0f;//위젯 데드 타이머 시간(위젯의 지속시간)
	...
	CharacterStat->SetCurrentHP(1500.0f);//HP스탯지정
	CharacterStat->SetMaxHP(1500.0f);//max HP스탯지정
...
void AShooterBossCharacter::SetCharacterState(ECharacterState NewState)
{
	...
	switch (CurrentState)
	{
		...
		case ECharacterState::DEAD://죽음 상태
		{
			...
			GetWorld()->GetTimerManager().SetTimer(DeadTimerHandle, FTimerDelegate::CreateLambda([this]()->void {//3초뒤 게임 클리어 화면 표시
					for (FConstPawnIterator It = GetWorld()->GetPawnIterator(); It; It++)//게임완료시 이터레이터를 이용하여 플레이어 폰 및 NPC 폰 비활성화
					{
						(*It)->TurnOff();
					}
					for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; It++)//게임완료시 이터레이터를 이용하여 플레이어 컨트롤러를 찾아 결과화면 출력
					{
...
void AShooterBossCharacter::Fire()//총알 발사
{
	...
	if (HitRes.bBlockingHit)//결과 인스턴스 유효할 시 
	{
		...
		if (HitRes.Actor.IsValid())
		{
			...
			HitRes.Actor->TakeDamage(CharacterStat->GetAIBossShootDamage(), DamageEvent, GetController(), this);
		}
	}
}
