...
#include "ShooterBossCharacter.h"//A 보스
...
AShooterAICharacter::AShooterAICharacter()
{
	...
	WidgetDeadTimer = 3.0f;//위젯 데드 타이머(위젯 지속시간)
...
void AShooterAICharacter::SetCharacterState(ECharacterState NewState)
{
	...
	case ECharacterState::READY:
	{
		...
		//HP 0일 떄 델리게이트를 사용하여 DEAD 스테이트로 설정
...
void AShooterAICharacter::Fire()
{
	...
	if (CharacterStat->GetMagazineBullet()>0)
	{
		...
		if (HitRes.bBlockingHit) 
		{
			...
			{
				// 플레이어와 타깃의 방향을 설정
				FVector ImpulseDirection = HitRes.GetActor()->GetActorLocation() - GetActorLocation();//밀어질 방향 설정(벡터 뺄셈활용)
			...
			{
				if (CollisionName2 == HitRes.GetComponent()->GetCollisionProfileName())//콜리전이 AI일때만
				{
...
void AShooterAICharacter::FCanAttack()//공격가능
{
	CanAttack = true;
}

void AShooterAICharacter::FCannotAttack()//공격불가능
{
	CanAttack = false;
}
...
float AShooterAICharacter::TakeDamage(float DamageAmount, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, AActor* DamageCauser)
{
	...
	ShooterAIController->TrackToTargetOutRange(DamageCauser->GetActorLocation());//피격시 AI 컨트롤러가 공격주체의 위치값을 가져와 그곳으로 이동하게끔 함
